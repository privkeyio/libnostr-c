...
EOF
        ...
        for header in "${headers_to_fix[@]}"; do
            ...
            if [[ -f "$header_file" ]] && ! grep -q "openssl_compat_fix.h" "$header_file"; then
                ...
                # Add include at the very top - portable sed approach
                sed -i .bak '1i\
 ...
 * MUST be included before any OpenSSL headers
 */
...
EOF
    ...
    # Inject the compatibility macros directly into key OpenSSL provider files
    # This ensures the macros are defined before any system OpenSSL headers are included
    ...
    local nc_crypto="$noscrypt_dir/src/nc-crypto.c"
    
    # Add compatibility macros to the main crypto file
    if [[ -f "$nc_crypto" ]]; then
        if ! grep -q "DEPRECATEDIN_1_1_0" "$nc_crypto"; then
            log_debug "Adding OpenSSL compatibility macros to nc-crypto.c"
            # Create a temporary file with the macros added at the top
            {
                echo "/* OpenSSL system compatibility macros - added by noscrypt header fix script */"
                echo "#ifndef DEPRECATEDIN_1_1_0"
                echo "#define DEPRECATEDIN_1_1_0(f) f"
                echo "#endif"
                echo "#ifndef DEPRECATEDIN_1_0_2"
                echo "#define DEPRECATEDIN_1_0_2(f) f"
                echo "#endif"
                echo "#ifndef DEPRECATEDIN_1_0_1"
                echo "#define DEPRECATEDIN_1_0_1(f) f"
                echo "#endif"
                echo "#ifndef DEPRECATEDIN_1_0_0"
                echo "#define DEPRECATEDIN_1_0_0(f) f"
                echo "#endif"
                echo "#ifndef DEPRECATEDIN_3_0"
                echo "#define DEPRECATEDIN_3_0(f) f"
                echo "#endif"
                echo "#ifndef DEPRECATEDIN_0_9_8"
                echo "#define DEPRECATEDIN_0_9_8(f) f"
                echo "#endif"
                echo ""
                cat "$nc_crypto"
            } > "$nc_crypto.tmp"
            mv "$nc_crypto.tmp" "$nc_crypto"
        fi
    fi
    
    # Add compatibility macros to OpenSSL provider files
    for provider_file in "$openssl_provider" "$openssl_helpers"; do
        if [[ -f "$provider_file" ]]; then
            if ! grep -q "DEPRECATEDIN_1_1_0" "$provider_file"; then
                log_debug "Adding OpenSSL compatibility macros to $(basename "$provider_file")"
                # Create a temporary file with the macros added at the top
                {
                    echo "/* OpenSSL system compatibility macros - added by noscrypt header fix script */"
                    echo "#ifndef DEPRECATEDIN_1_1_0"
    ...
    log_debug "System OpenSSL compatibility macros injected directly into source files"
}
